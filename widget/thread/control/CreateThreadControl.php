...
class createThreadControl{
    ...
    public static function createKatalogi(User $user, $title, $cover, $desc, $subtype, $extra, UserManager $um, PostManager $pm, PointManager $lm, $path=""){
        global $hash;
        $limit = self::hasLimit($user,$lm);
        if($limit != 0)
            return $limit;
            if(!preg_match(Constant::REGEX_FORMAT_TITLE, $title))
                return Constant::ERROR_CODE_THREAD_TITLE;
            ...
            switch($verif){
                ...
                case 1:
                    ...
                    $post->setType(Constant::THREAD_KATALOGI);
                    $post->addData(["title"=>$title]);

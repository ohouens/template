...
class createThreadControl{
    ...
    public static function hasLimit(User $user, PointManager $lm){
        if(count(array_diff($user->getData()["threads"], $user->getData()["starter"])) + count($user->getData()['posters']) >= self::LIMIT + $user->getData()["slots"] and !in_array($user->getId(), self::UNLIMITED))
            return true;
        return false;
    }

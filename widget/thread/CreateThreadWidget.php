...
class CreateThreadWidget extends Widget{
    ...
    private function subConstruct(User $user, PointManager $lm){
        ...
        '<div id="createThread">
            ...
            <div id="takenSlots">Taken slots: '.(count(array_diff($user->getData()["threads"], $user->getData()["threads"]))+count($user->getData()['posters'])).' / '.($user->getData()['slots']+CreateThreadControl::LIMIT).'</div>
        </div>
        ';
    }
}
